...
if (isset($_GET['order_id'])) {
    ...
    if ($stmt = $conn->prepare($query)) {
        ...
        if ($result->num_rows > 0) {
            ...
            echo '<thead><tr><th style="text-align: center;">Product</th><th style="text-align: center;">Amount</th><th style="text-align: center;">Detail</th><th style="text-align: center;">Status</th><th style="text-align: center;">Action</th></tr></thead>';
            echo '<tbody>';
            while ($detail = $result->fetch_assoc()) {
                ...
                echo '<td align="center">
                        <button class="btn btn-success allow-btn-1" data-cart-detail-id="' . htmlspecialchars($detail['cart_detail_id']) . '" data-status-id="' . htmlspecialchars($detail['cart_status_id']) . '">Allow</button>
                        <button class="btn btn-danger deny-btn-1" data-cart-detail-id="' . htmlspecialchars($detail['cart_detail_id']) . '" data-status-id="' . htmlspecialchars($detail['cart_status_id']) . '">Deny</button>
                      </td>';
                echo '</tr>';
            }
            echo '</tbody></table>';
